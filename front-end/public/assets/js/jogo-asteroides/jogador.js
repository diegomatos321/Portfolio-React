...
export default class Jogador extends GameObject {
    ...
    rotateToMouse(mousePos) {
        let canvasPosicao = this.getParentCoords(this.scene.parentElement)

        let gameRatio = this.scene.GAME_WIDTH / this.scene.displayWidth;

        let mousePosition = mousePos;        
        mousePosition.subtractBy(canvasPosicao);
        mousePosition.multiplyBy(gameRatio);

        this._direction = mousePosition.subtract(this._position);
    }

    getParentCoords(parent){
        let parentPosicao = parent.getBoundingClientRect();

        let novoX = parentPosicao.left;
        let novoY = parentPosicao.top;
